...
jQuery(document).ready(function ($) {
	  /*
	  if($('#link-bds-ref').length)
	  {
		
		  document.getElementById("link-bds-ref").value = window.location.href;
	  }*/
	 if($('#unit_price').length)
	 {
		  $("#price, #re_area, #re_front, #re_row ").inputFilter(function(value) {
    return /^\d*$/.test(value);  
  },"Chỉ cho phép gõ số vào ô này.");
...
if($('.mh-main-slider').length){
  $('.mh-main-slider').lazyFlickity({
    // options
    cellAlign: 'center',
    contain: true,
    wrapAround: true,
...
if($('.mh-nav-slider').length){
	$('.mh-nav-slider').lazyFlickity({
...
if($('.mh-carousel').length){
  ...
  if($('#table-list-post').length){
	  $('#table-list-post tr td a.action.delete').click(function(){
		  $(this).prop('disabled', true);
		  var re_id = $(this).data('id');
		  if (confirm('Thao tác xóa tin đăng sẽ không hồi phục lại được. Bạn có chắc chắn xóa tin này?')) {
			   $.ajax({
      type: 'GET',
      url: flatsomeVars.ajaxurl,
      data: {

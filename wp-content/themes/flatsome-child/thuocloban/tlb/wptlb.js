...
function pullRightAction () {
	...
	if(trimStart > 0) {
		...
		var newLi = jQuery('<li>').append(jQuery('<img/>', { src: thuoc522_url + qStr }))
				.append(jQuery('<img/>', { src: thuoc429_url + qStr }))
				.append(jQuery('<img/>', { src: thuoc388_url + qStr }));
...
function getNoiDungHienThi(valcm) {
	for (var i = 0; i < 3; i++) {
		...
		jQuery('#nd' + ddt + '-text').html(result);
	}
}

jQuery( document ).ready(function() {
	jQuery('.lazy-load-lb').each(function(i, obj) {
	
	var lzHinhAnh = jQuery(obj).data('src');
	jQuery(obj).attr("src",lzHinhAnh);
    });
});

...
define( 'WPTLB_URI', get_stylesheet_directory_uri().'/thuocloban/' );
define( 'WPTLB_VERSION', '1.0' );
add_action( 'init', 'wptlb_init' );
add_action( 'wp_enqueue_scripts', 'wptlb_enqueue_scripts' );
function wptlb_enqueue_scripts() {
	wp_enqueue_style( 'wptlb_iframe', WPTLB_URI . 'tlb/wptlb_iframe.css' );
	wp_enqueue_script( 'wptlb_iframe', WPTLB_URI . 'tlb/wptlb_iframe.js', array( 'jquery' ), WPTLB_VERSION, true );
}

function wptlb_init() {
	// shortcode
	add_shortcode( 'wp_thuocloban', 'wptlb_shortcode_thuocloban' );
	add_shortcode( 'wp_thuocloban_iframe', 'wptlb_shortcode_thuocloban_iframe' );
}
function fnTLBURL(){
	echo '<script> var thuoc522_url = "'.  WPTLB_URI . 'tlb/thuoc522.php"; var thuoc429_url = "'.  WPTLB_URI . 'tlb/thuoc429.php"; var thuoc388_url = "'.  WPTLB_URI . 'tlb/thuoc388.php";</script>';
}
add_action('wp_head', 'fnTLBURL' );	
function wptlb_shortcode_thuocloban() {
	ob_start();

wp_enqueue_style( 'wptlb_css', WPTLB_URI . 'tlb/wptlb.css' );	
wp_enqueue_script( 'wptlb-scroll', WPTLB_URI . 'tlb/iscroll.js', array( 'jquery' ), WPTLB_VERSION, true );	
wp_enqueue_script( 'wptlb', WPTLB_URI . 'tlb/wptlb.js', array( 'jquery' ), WPTLB_VERSION, true );
	?>
	
	
	<div class="wp_thuocloban">
		<div id="main">
			<div class="container">
				<div class="content">
					<div id="lobanOuter" style="height:400px;">
						...
						<div id="container-sodo"><input type="text" value="0" name="sodo" id="sodo" /> mm (nhập số)
						</div>
						<div id="thanhdo"></div>
						<p class="loban-note">Hãy kéo thước</p>
						<p class="loban-t loban-522">
							<strong>Thước Lỗ Ban 52.2cm:</strong> Khoảng thông thủy (cửa, cửa sổ...)</p>
						<p class="loban-t loban-429">
							<strong>Thước Lỗ Ban 42.9cm (Dương trạch):</strong> Khối xây dựng (bếp, bệ, bậc...)</p>
						<p class="loban-t loban-388">
							<strong>Thước Lỗ Ban 38.8cm (Âm phần):</strong> Đồ nội thất (bàn thờ, tủ...)</p>
						<div id="loban-wrapper">
							<div id="loban-scroller">
								...
								<ul id="loban-thelist">
									<li>
										<img class="lazy-load-lb"  src="data:image/svg+xml,%3Csvg%20viewBox%3D%220%200%20680%20453%22%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%3E%3C%2Fsvg%3E"  data-src="<?php echo WPTLB_URI; ?>tlb/thuoc522.php" nopin="nopin"  loading="lazy"  />
										<img class="lazy-load-lb"  src="data:image/svg+xml,%3Csvg%20viewBox%3D%220%200%20680%20453%22%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%3E%3C%2Fsvg%3E"   data-src="<?php echo WPTLB_URI; ?>tlb/thuoc429.php" nopin="nopin"  loading="lazy"  />
										<img class="lazy-load-lb"  src="data:image/svg+xml,%3Csvg%20viewBox%3D%220%200%20680%20453%22%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%3E%3C%2Fsvg%3E"   data-src="<?php echo WPTLB_URI; ?>tlb/thuoc388.php" nopin="nopin"  loading="lazy"  />
									</li>
								</ul>
					...
					<div style="text-align: right; padding-top: 0px; padding-bottom: 10px">Đơn vị tính: mm</div>
				</div>
			</div>
			...
			<div id="thuocloban">
				<p style="margin: 20px 0 12px 0"><strong>Bảng tra nhanh thước Lỗ Ban 52.2</strong></p>
				<div id="loban">
					<div class="loban-h">
	...
	<?php
	$tlb_output = ob_get_contents();
	ob_end_clean();

	return $tlb_output;
}

function wptlb_shortcode_thuocloban_iframe() {
	return '<iframe src="' . WPTLB_URI . 'tlb/index.php" class="wp_thuocloban_iframe" frameBorder="0"></iframe>';
}